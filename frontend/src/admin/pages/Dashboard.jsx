...
export default function Dashboard() {
  ...
  const fetchStats = useCallback((signal) => {
    adminApi.get("/admin/dashboard/stats", { signal })
      .then((r) => {
        setStats(r.data);
  ...
  useEffect(() => {
    ...
    fetchStats(controller.signal); // load immediately on mount
              ...
              {[
                ...
                <div key={row.label} style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ height: 6, background: tokens.border, borderRadius: tokens.radius.pill, overflow: "hidden" }}>
                    <div
                      style={{
                        width: stats.total_orders ? `${(row.value / stats.total_orders) * 100}%` : "0%",
                        height: "100%",
                        background: row.color,
                        borderRadius: tokens.radius.pill,
                      }}
