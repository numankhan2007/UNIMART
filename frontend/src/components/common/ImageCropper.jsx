...
export default function ImageCropper({
  ...
  const drawPreview = useCallback(() => {
    ...
    ctx.drawImage(imageObj, position.x, position.y, scaledWidth, scaledHeight);
  ...
  const handleCrop = () => {
    ...
    outputCtx.drawImage(imageObj, sourceX, sourceY, sourceWidth, sourceHeight, 0, 0, outputWidth, outputHeight);
    outputCanvas.toBlob(
      (blob) => {
        if (blob) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm modal-backdrop-enter"
      onClick={onCancel}
    >
      <div
        className="w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden modal-enter"
