...
export function OrderProvider({ children }) {
  ...
  const getOrdersByBuyer = useCallback(async (signal) => {
    const response = await orderService.getByBuyer(signal);
    setOrders(response.data);
    return response.data;
  }, []);

  const getOrdersBySeller = useCallback(async (signal) => {
    const response = await orderService.getBySeller(signal);
    return response.data;
  }, []);

  const getOrderById = useCallback(async (orderId, signal) => {
    const response = await orderService.getById(orderId, signal);
    return response.data;
  }, []);
