...
if is_production():
    REFRESH_SECRET = require_secret("REFRESH_TOKEN_SECRET", min_len=32)
...
OTP_HMAC_KEY = os.getenv("OTP_HMAC_KEY", "change-otp-hmac-key").encode()


def _hash_otp(otp: str) -> str:
    return _hmac.new(OTP_HMAC_KEY, otp.encode(), hashlib.sha256).hexdigest()
...
@router.get(
    "/verify/{register_number}",
    response_model=OfficialRecordResponse,
    dependencies=[Depends(rate_limit_strict(20, 60))],
)
...
async def send_registration_otp(
    ...
    otp_key = f"reg_otp:{data.email}"
    redis_client.setex(otp_key, OTP_EXPIRY_SECONDS, _hash_otp(otp))
...
def verify_registration_otp(data: RegistrationOTPVerify):
    ...
    if not _hmac.compare_digest(stored_otp, _hash_otp(otp_value)):
